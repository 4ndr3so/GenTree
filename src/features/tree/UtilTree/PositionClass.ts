...
export class PositionClass {
    ...
    public posicionarRoot(persona: Person): Person[] {
        let clone=this.clonePerson(persona, new Map<string, Person>());
    ...
    public posicionarPareja(persona: Person): Person[] {
        
        const map = new Map<string, Person>();
        persona = this.clonePerson(persona, map);
       
        const currentPartner = persona.relacion.getCurrentPartner();
        console.log("persona", currentPartner);
        if (!currentPartner) return [persona];
        
        const refPos = currentPartner.getPosition();
        console.log("refPos", refPos);
        persona.setPosition(refPos.x + 100, refPos.y);
        return [...this.people, persona];

    }


    public posicionarHijo(persona: Person): Person[] {
        const [p1, p2] = persona.relacion.getParents();
        this.reorganizarHijosConRelaciones([p1, p2], persona);
        return this.detectarColision(this.people, persona);
    }
    ...
    public reorganizarHijosConRelaciones(parents: [Person | null, Person | null], includeNew?: Person): void {
        const [p1, p2] = parents;
        const rawChildren = [...(p1?.relacion.getChildren() ?? []), ...(p2?.relacion.getChildren() ?? []), ...(includeNew ? [includeNew] : [])];
        const hijosUnicos = Array.from(new Map(rawChildren.map(p => [p.id, p])).values()).sort((a, b) => a.id.localeCompare(b.id));
        if (hijosUnicos.length === 0) return;

        const centroX = p1 && p2 ? (p1.positionX + p2.positionX) / 2 : (p1?.positionX || p2?.positionX || window.innerWidth / 2);
    ...
    public isPositionOccupiedBySibling(x: number, y: number, siblings: Person[]): boolean {
