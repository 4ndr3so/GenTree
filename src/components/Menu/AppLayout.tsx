import React, { Children, useState, type ReactElement } from "react";
...
import type { ShapeType } from "../../features/tree/NodeShape";
// ajusta el path si es necesario


type AppLayoutProps = {
  children: ReactElement<{ shapeType?: ShapeType }>; // ✅ Aquí se declara shapeType como prop esperada
};
...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  const [shapeType, setShapeType] = useState<ShapeType>("grand");
  ...
  return (
    <>
      <div className="grid grid-cols-4 h-screen">
        {/* Sidebar */}
        <aside className="col-span-1 bg-indigo-600 text-white flex flex-col">
          ...
          <nav className="flex-1 px-4 space-y-2">
            ...
            <select onChange={(e) => setShapeType(e.target.value as ShapeType)} className="w-full px-3 py-2 rounded-md border text-sm bg-indigo-700 text-white">
        ...
        {/* Main content */}
        <main className="col-span-3 flex flex-col bg-white">
          {/* Topbar */}
          ...
          {/* Content area */}
          <section className="flex-1 relative overflow-hidden border border-dashed border-gray-400">
            {React.isValidElement(children)
              ? React.cloneElement(children, { shapeType }) // <-- Aquí se pasa
              : children}
